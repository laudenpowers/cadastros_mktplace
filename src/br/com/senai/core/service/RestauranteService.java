		...
		if (restaurante != null) {
			...
			if (restaurante.getEndereco() != null) {
				...
				if (restaurante.getCategoria() != null && restaurante.getCategoria().getId() > 0) {
					...
					if (isCidadeInvalida) {
						throw new IllegalArgumentException("A cidade do endereço é obrigatória e não "
					...
					if (isBairroInvalido) {
						throw new IllegalArgumentException("O bairro do endereço é obrigatório e não "
	...
	public Restaurante buscarPor(int idDoRestaurante) {
		if (idDoRestaurante > 0) {
			...
			if (restauranteEncontrado) {
				throw new IllegalArgumentException("Não existe restaurante vinculado ao id "
						+ "informado");
			}
